 ...
 * @since 19.12.2014
 */
public class CommonTransactionExecutor implements TransactionExecutor {

    protected static final Logger logger = LoggerFactory.getLogger("execute");
    ...
    protected ByteArraySet touchedAccounts = new ByteArraySet();

    protected boolean localCall = false;

    public CommonTransactionExecutor(Transaction tx, byte[] coinbase, Repository track, BlockStore blockStore,
    ...
    protected boolean isSignatureValid() {
        return blockchainConfig.acceptTransactionSignature(tx);
    }
