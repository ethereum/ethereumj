 ...
 * For initializing the contract and its address use the [setContract] method
 *
...
public abstract class EventListener<EventData> {
    ...
    protected LogFilter contractAddressFilter;
    ...
    public void onBlockImpl(BlockSummary blockSummary) {
        try {
            ...
            if (bestBlock != null && blockSummary.getBlock().getNumber() < bestBlock.getNumber() - 16) {  // FIXME: why 16?
                logger.debug("Ignoring block as too old: " + blockSummary.getBlock().getShortDescr());
                return;
            }

            if (contractAddressFilter.matchBloom(new Bloom(blockSummary.getBlock().getLogBloom()))) {
                for (int i = 0; i < blockSummary.getReceipts().size(); i++) {
                    TransactionReceipt receipt = blockSummary.getReceipts().get(i);
                    if (contractAddressFilter.matchBloom(receipt.getBloomFilter())) {
                        if (!pendings.containsKey(new ByteArrayWrapper(receipt.getTransaction().getHash()))) {
    ...
    public void onPendingTransactionUpdateImpl(TransactionReceipt txReceipt, PendingTransactionState state, Block block) {
        try {
            if (state != PendingTransactionState.DROPPED || pendings.containsKey(new ByteArrayWrapper(txReceipt.getTransaction().getHash()))) {
    ...
    /**
     *  Initialize listener with the contract of interest
     */
    public void setContract(String abi, byte[] contractAddress) {
        contract = new CallTransaction.Contract(abi);
        contractAddressFilter = new LogFilter().withContractAddress(contractAddress);
    }
    ...
    private synchronized void onReceipt(TransactionReceipt receipt, Block block, PendingTransactionState state) {
        ...
        if (contractAddressFilter.matchBloom(receipt.getBloomFilter())) {
            ...
            for (LogInfo logInfo : receipt.getLogInfoList()) {
                if (contractAddressFilter.matchBloom(logInfo.getBloom()) &&
                        contractAddressFilter.matchesExactly(logInfo)) {
        ...
        } else if (state == PendingTransactionState.DROPPED && pendings.containsKey(txHashW)) {
            PendingEvent event = pendings.get(txHashW);
    ...
    private void onEventData(TransactionReceipt receipt, Block block,
        ...
        ByteArrayWrapper txHashW = new ByteArrayWrapper(receipt.getTransaction().getHash());
        PendingEvent espTx = pendings.get(txHashW);
        boolean newEvent = false;
        if (espTx == null) {
            // new Tx
            espTx = new PendingEvent(state.isPending() ? Utils.toUnixTime(System.currentTimeMillis()) : block.getTimestamp());
            pendings.put(txHashW, espTx);
            newEvent = true;
        }
        ...
        espTx.update(receipt, matchedTxs, state, block);
        logger.debug("Event " + (newEvent ? "created" : "updated") + ": " + espTx);

        if (pendingTransactionUpdated(espTx)) {
            pendings.remove(txHashW);
        }
        pendingTransactionsUpdated();
    }

    private EventData onLogMatch(LogInfo logInfo, Block block, TransactionReceipt receipt, int txCount, PendingTransactionState state) {
        ...
        for (PendingEvent event : pendings.values()) {
            if (event.setBestConfirmingBlock(newBlock)) {
                ...
                if (remove) {
                    ...
                    toRemove.add(new ByteArrayWrapper(event.receipt.getTransaction().getHash()));
                }
                updated = true;
            }
        }
