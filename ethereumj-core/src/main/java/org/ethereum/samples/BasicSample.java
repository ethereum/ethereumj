...
public class BasicSample implements Runnable {
        ...
        while (true) {
            ...
            if (cnt > 50) {
                logger.error("Looks like discovery failed, no nodes were found.\n" +
                                     ...
                                     "if this interface is correct, otherwise set it manually via 'peer.discovery" +
                                     ".bind.ip' option.");
    ...
    protected void waitForFirstBlock() throws Exception {
        ...
        while (true) {
            ...
            if (cnt > 330) {
                logger.error("No blocks imported during a long period. Must be a problem, logs need to be " +
                                     "investigated.");
