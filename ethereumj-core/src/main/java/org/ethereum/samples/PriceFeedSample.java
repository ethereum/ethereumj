...
public class PriceFeedSample extends BasicSample {
        ...
        while (true) {
            if (priceFeedContract.isExist()) {
                ...
                for (String ticker : tickers) {
                    s += ticker + " " + priceFeedContract.getPrice(ticker) + " (" +
                            priceFeedContract.getTimestamp(ticker) + "), ";
                }
                logger.info(s);
            } else {
    ...
    abstract class EthereumContract {
        ...
        protected Object[] callFunction(String name, String[] inParamTypes, String[] outParamTypes, Object... args) {
            CallTransaction.Function function =
                    CallTransaction.Function.fromSignature(name, inParamTypes, outParamTypes);
    ...
    class NameRegContract extends EthereumContract {
        ...
        public byte[] addressOf(String name) {
            BigInteger bi =
                    (BigInteger) callFunction("addressOf", new String[]{"bytes32"}, new String[]{"address"}, name)[0];
    ...
    class PriceFeedContract extends EthereumContract {
        ...
        private static final String contractABI =
                "[{" + "  'constant': true," + "  'inputs': [{" + "    'name': 'symbol'," + "    'type': 'bytes32'" +
                        "  }]," + "  'name': 'getPrice'," + "  'outputs': [{" + "    'name': 'currPrice'," +
                        "    'type': 'uint256'" + "  }]," + "  'type': 'function'" + "}, {" + "  'constant': true," +
                        "  'inputs': [{" + "    'name': 'symbol'," + "    'type': 'bytes32'" + "  }]," +
                        "  'name': 'getTimestamp'," + "  'outputs': [{" + "    'name': 'timestamp'," +
                        "    'type': 'uint256'" + "  }]," + "  'type': 'function'" + "}, {" + "  'constant': true," +
                        "  'inputs': []," + "  'name': 'updateTime'," + "  'outputs': [{" + "    'name': ''," +
                        "    'type': 'uint256'" + "  }]," + "  'type': 'function'" + "}, {" + "  'inputs': []," +
                        "  'type': 'constructor'" + "}]";
