...
public class TreeChunker implements Chunker {
    ...
    public class TreeChunk extends Chunk {
        ...
        public String toString() {
            String dataString =
                    ByteUtil.toHexString(Arrays.copyOfRange(getData(), getDataOffset(), getDataOffset() + 16)) + "...";
    ...
    private class LazyChunkReader implements SectionReader {
        ...
        private int readImpl(byte[] dest, int destOff, Chunk chunk, long chunkWidth, long chunkStart, long readStart,
                             long readEnd) {
            ...
            if (chunkWidth > chunkSize) {
                ...
                for (int i = startSubChunk; i <= lastSubChunk; i++) {
                    ...
                    ret += readImpl(dest,
                                    (int) (destOff + (i - startSubChunk) * subChunkWidth),
                                    subChunk,
                                    subChunkWidth,
                                    chunkStart + i * subChunkWidth,
                                    readStart,
                                    readEnd);
                }
            } else {
                ...
                System.arraycopy(dataChunk.getData(),
                                 (int) (dataChunk.getDataOffset() + chunkReadStart),
                                 dest,
                                 destOff,
                                 ret);
            }
