...
//    private Map<Node, Date> expectedPongs = new HashMap<>();
    ...
    private Map<Node, NodeHandler> nodeHandlerMap = new HashMap<>();
...
//    }
    ...
    void sendMessage(NodeHandler handler, Message msg) {
        InetSocketAddress address = handler.getInetSocketAddress();
        logger.trace(" <===({}) {} [{}] {}", address, msg.getClass().getSimpleName(), this, msg);
        sendPacket(handler, msg.getPacket(), address);
    }
    synchronized void sendPacket(NodeHandler handler, byte[] wire, InetSocketAddress address) {
