...
public class Eth63 extends Eth62 {
    ...
    protected synchronized void processGetNodeData(GetNodeDataMessage msg) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("Peer {}: processing GetNodeData, size [{}]",
    ...
    protected synchronized void processGetReceipts(GetReceiptsMessage msg) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("Peer {}: processing GetReceipts, size [{}]",
                         channel.getPeerIdShort(),
    ...
    protected synchronized void processNodeData(NodeDataMessage msg) {
        ...
        for (Value nodeVal : msg.getDataList()) {
            ...
            if (!requestedNodes.contains(hash)) {
                String err =
                        "Received NodeDataMessage contains non-requested node with hash :" + Hex.toHexString(hash) +
                                " . Dropping peer " + channel;
                dropUselessPeer(err);
                return;
            }
    ...
    protected synchronized void processReceipts(ReceiptsMessage msg) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("Peer {}: processing Receipts, size [{}]", channel.getPeerIdShort(), msg.getReceipts().size());
        }
    ...
    public String getSyncStats() {
        double nodesPerSec = 1000d * channel.getNodeStatistics().eth63NodesReceived.get() /
                channel.getNodeStatistics().eth63NodesRetrieveTime.get();
        double missNodesRatio = 1 - (double) channel.getNodeStatistics().eth63NodesReceived.get() /
                channel.getNodeStatistics().eth63NodesRequested.get();
