...
public class IndexedBlockStore extends AbstractBlockstore {
            ...
            new Serializer<List<BlockInfo>, byte[]>() {
                ...
                public byte[] serialize(List<BlockInfo> value) {
                    ...
                    for (BlockInfo blockInfo : value) {
                        ...
                        if (blockInfo.getCummDifficulty() == null ||
                                blockInfo.getCummDifficulty().compareTo(BigInteger.ZERO) < 0) {
                ...
                public List<BlockInfo> deserialize(byte[] bytes) {
                    ...
                    for (RLPElement element : list) {
                        ...
                        blockInfo.setCummDifficulty(
                                rlpCummDiff == null ? BigInteger.ZERO : ByteUtil.bytesToBigInteger(rlpCummDiff));
    ...
    public synchronized byte[] getBlockHashByNumber(long blockNumber) {
        ...
        return chainBlock == null ? null :
                chainBlock.getHash(); // FIXME: can be improved by accessing the hash directly in the index
    }
    ...
    private void addInternalBlock(Block block, BigInteger cummDifficulty, boolean mainChain) {
        ...
        blockInfo.setMainChain(mainChain); // FIXME:maybe here I should force reset main chain for all uncles on that
        // level
