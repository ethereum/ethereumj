...
public class Miner {
    ...
    public boolean mine(Block newBlock, byte[] difficulty) {
        ...
        byte[] target = BigIntegers.asUnsignedByteArray(32, max.divide(new BigInteger(1, difficulty)));


        long newGasLimit = Math.max(125000,
                                    (new BigInteger(1, newBlock.getGasLimit()).longValue() * (1024 - 1) +
                                            (newBlock.getGasUsed() * 6 / 5)) / 1024);
