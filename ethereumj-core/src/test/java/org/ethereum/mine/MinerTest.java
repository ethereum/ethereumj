...
public class MinerTest {
    ...
    Map<ByteArrayWrapper, Pair<Transaction, Long>> submittedTxs =
            Collections.synchronizedMap(new HashMap<ByteArrayWrapper, Pair<Transaction, Long>>());
    ...
    public void startMiningConsumer() throws Exception {
        SysPropConfig2.props.overrideParams(ConfigFactory.parseString("peer.listen.port = 30336 \n" +
                                                                              "peer.privateKey = " +
                                                                              "3ec771c31cac8c0dba77a69e503765701d3c2bb62435888d4ffa38fed60c445c \n" +
                                                                              "peer.networkId = 555 \n" +
                                                                              "peer.active = [" +
                                                                              "{ url = " +
                                                                              "\"enode://b23b3b9f38f1d314b27e63c63f3e45a6ea5f82c83f282e2d38f2f01c22165e897656fe2e5f9f18616b81f41cbcf2e9100fc9f8dad099574f3d84cf9623de2fc9@localhost:20301\" }," +
                                                                              "{ url = " +
        ...
        ethereum2.addListener(new EthereumListenerAdapter() {
            ...
            public void onBlock(Block block, List<TransactionReceipt> receipts) {
                ...
                for (Transaction tx : block.getTransactionsList()) {
                    //                    Pair<Transaction, Long> remove = submittedTxs.remove(new ByteArrayWrapper
                    // (tx.getHash()));
                    //                    if (remove == null) {
                    //                        System.err.println("===== !!! Unknown Tx: " + tx);
                    //                    } else {
                    //                        System.out.println("===== Tx included in " + (System.currentTimeMillis
                    // () - remove.getRight()) / 1000
                    //                                + " sec: " + tx);
                    //                    }
                ...
                //                        System.err.println("==== !!! Lost Tx: " + (System.currentTimeMillis() -
                // pair.getRight()) / 1000
                //                                + " sec: " + pair.getLeft());
                //                    }
                //                }
        ...
        //        ECKey senderKey = ECKey.fromPrivate(Hex.decode
        // ("3ec771c31cac8c0dba77a69e503765701d3c2bb62435888d4ffa38fed60c445c"));
    ...
    public void startMiningTest() throws FileNotFoundException, InterruptedException {
        SysPropConfig1.props.overrideParams(ConfigFactory.parseString("peer.listen.port = 30335 \n" +
                                                                              "peer.privateKey = " +
        ...
        for (int i = ethereum1.getRepository().getNonce(senderKey.getAddress()).intValue(), j = 0; j < 20000;
             i++, j++) {
            {
                Transaction tx = new Transaction(ByteUtil.intToBytesNoLeadZeroes(i),
                                                 ByteUtil.longToBytesNoLeadZeroes(50_000_000_000L),
    ...
    public void blockTest() {
        String rlp =
                "f90498f90490f90217a0887405e8cf98cfdbbf5ab4521d45a0803e397af61852d94dc46ca077787088bfa0d6d234f05ac90931822b7b6f244cef81" +
    ...
    public void blockTest1() {
        String rlp =
                " |  | " +
