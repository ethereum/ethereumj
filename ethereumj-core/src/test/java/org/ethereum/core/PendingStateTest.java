...
public class PendingStateTest {
    ...
    public void testRebranch1() throws InterruptedException {
        ...
        pendingState.addPendingTransaction(tx1);
        Transaction tx2 =
                bc.createTransaction(charlie, 0, alice.getAddress(), BigInteger.valueOf(1000000), new byte[0]);
        ;
        pendingState.addPendingTransaction(tx2);
    ...
    public void testPrevBlock() throws InterruptedException {
        ...
        SolidityContract contract = bc.submitNewContract(
                "contract A {" + "  function getPrevBlockHash() returns (bytes32) {" +
    ...
    static class PendingListener extends EthereumListenerAdapter {
        ...
        //        public BlockingQueue<Triple<TransactionReceipt, PendingTransactionState, Block>>
        // onPendingTransactionUpdate = new LinkedBlockingQueue<>();
        ...
        public void onPendingTransactionUpdate(TransactionReceipt txReceipt, PendingTransactionState state,
            ...
            System.out.println(
                    "PendingStateTest.onPendingTransactionUpdate:" + "txReceipt.err = [" + txReceipt.getError() +
                            "], state = [" + state + "], block: " + block.getShortDescr());
            getQueueFor(txReceipt.getTransaction()).add(Triple.of(txReceipt, state, block));
        }

        public synchronized BlockingQueue<Triple<TransactionReceipt, PendingTransactionState, Block>> getQueueFor(
                Transaction tx) {
            ByteArrayWrapper hashW = new ByteArrayWrapper(tx.getHash());
            BlockingQueue<Triple<TransactionReceipt, PendingTransactionState, Block>> queue =
                    onPendingTransactionUpdate.get(hashW);
            if (queue == null) {
        ...
        public Triple<TransactionReceipt, PendingTransactionState, Block> pollTxUpdate(Transaction tx)
